...
cat var/log/./* | awk -F: 'match($0, /([A-Za-z0-9 ]+):([A-Za-z0-9 : \[ \] ]+) (Failed password) ([a-zA-Z0-9 ]+) ([a-zA-Z0-9]+) from([[0-9 \.]+) /, groups) {print groups[1]" "groups[5]groups[6]}' | sed 's/[ ]\+/ /g' > failed_login_data.txt
